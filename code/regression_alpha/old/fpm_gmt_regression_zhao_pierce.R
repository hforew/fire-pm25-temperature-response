...
##   Combined: 7 observations per country for the regression.
##
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# Drop GIVE countries with no corresponding data in the Pierce or Zhao PM gridded data.
...
# Inspect the reshaped data to confirm structure (7 rows per country)
print(head(reg_data_long, 15))

# Sanity check: print total rows and a summary of per-country observation counts.
# Every country should have exactly 7 rows (5 Pierce + 2 Zhao).
# Min = Max = 7 confirms no duplicates and no missing data sources.
...
## across the 7 (period × scenario) data points (5 Pierce + 2 Zhao).
##
...
# Print the USA data to inspect all 7 observations (5 Pierce period × scenario + 2 Zhao)
# and their GMT values that will serve as inputs to the regression.
...
# across all 7 observations (5 Pierce + 2 Zhao).
...
reg_results <- reg_data_long %>%
  ...
  # in practice all countries have 7 observations after the pop_bar_c == 0 filter)
  filter(n() >= 2) %>%
...
#
# Degrees of freedom (DF): each country regression has n = 7 observations (5 Pierce +
#   2 Zhao) and estimates 2 parameters (intercept + slope), so DF = n - 2 = 5.
#
# With 5 DF, t_critical is ~2.571.
df <- nrow(usa_data)-2
t_critical <- qt(0.975, df)   # ~2.571 for 95% CI with 5 degrees of freedom
cat("\nt_critical (95% CI, df = 5):", round(t_critical, 4), "\n")

t_critical_largesample <- 1.96

reg_coefs <- reg_coefs %>%
  mutate(
    # 95% CI bounds using exact t_critical for df = 5
  ...
  select(
    ...
    t_critical_df5, t_critical_largesample,
...
write_csv(reg_coefs, here("output", "fpm_gmt_regression_coefs_zhao_pierce.csv"))

cat("\nSaved regression coefficients to output/fpm_gmt_regression_coefs_zhao_pierce.csv\n")
