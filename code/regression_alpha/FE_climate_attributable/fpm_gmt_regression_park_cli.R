# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## FPM–GMT RELATIONSHIP: Estimate beta^(c) (per-capita climate-attributable fire PM2.5
##                        change per 1°C GMT) — Park et al. historical data,
##                        one regression per fire model
##
## Goal: For each country c and each Park fire model m, estimate the linear regression:
##   PM_bar_cli^(c)_td = alpha^(c,m) + beta^(c,m) * T_td + epsilon^(c,m)_td
##
## where PM_bar_cli is per-capita climate-attributable fire PM2.5
##   (factual obsclim minus counterfactual counterclim, population-weighted),
## T_td is GMT change in decade d (°C relative to 1850–1900),
...
##
## Inputs:
##   pop_wght_pm_cntry_final.csv   (exposure_percap_park_{model}_{decade}_fpm_cli cols)
##   gmt_park_decades.csv          (GMT anomaly per Park decade)
##
## Outputs:
...
# country diagnostic: 18 rows (3 models x 6 decades), check exposure_percap and T_ps
print(park_data_long %>% filter(country_code_iso3 == "NGA"))
print(park_data_long %>% filter(country_code_iso3 == "AUS"))
print(park_data_long %>% filter(country_code_iso3 == "ARG"))

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
############ Helper: run regression for one Park fire model ################
##
...
write_csv(reg_coefs_classic, here("output", "fpm_gmt_regression_coefs_classic_cli.csv"))
write_csv(reg_coefs_jules,   here("output", "fpm_gmt_regression_coefs_jules_cli.csv"))
write_csv(reg_coefs_ssib4,   here("output", "fpm_gmt_regression_coefs_ssib4_cli.csv"))

cat("\nSaved regression coefficients to:\n")
cat("  output/fpm_gmt_regression_coefs_classic_cli.csv\n")
cat("  output/fpm_gmt_regression_coefs_jules_cli.csv\n")
cat("  output/fpm_gmt_regression_coefs_ssib4_cli.csv\n")
