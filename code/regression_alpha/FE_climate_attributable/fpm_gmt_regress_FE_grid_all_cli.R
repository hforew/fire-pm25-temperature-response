...
##   - Park et al.:  18 obs (3 fire models x 6 decades) — climate change-attributable output
...
##
## Inputs:
##   output/pop_pm_country_death_final.csv   (grid-cell fPM; Pierce, Park _fpm_cli, and Zhao cols)
...
##
## Outputs:
##   output/fpm_gmt_regression_coefs_FE_grid_cli.csv   (beta_i per grid cell with SE, p-value,
##                                                       lon, lat; used for spatial mapping)
##
## Execution order:
##   files run before: death_rate_processing.R              --> writes pop_pm_country_death_final.csv
##   files run after:  plots_beta_lobf_fe_grid_all_cli.R   --> reads fpm_gmt_regression_coefs_FE_grid_cli.csv
##
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#              Park (park_{model}_{decade}_fpm_cli: 18 columns)
...
park_fpm_cols <- grep("^park_(classic|jules|ssib4)_.+_fpm_cli$", colnames(grid), value = TRUE)
...
park_long <- grid %>%
  # select only the 18 Park fPM cli columns (excludes park_*_pop and non-fpm_cli cols)
  select(row_id, lon, lat, country_code_iso3, country_name,
         starts_with("park_") & ends_with("_fpm_cli")) %>%
  # pivot_longer stacks the 18 park fpm_cli columns into period_scenario + fpm
  pivot_longer(
...
write_csv(reg_coefs, here("output", "fpm_gmt_regression_coefs_FE_grid_cli.csv"))
cat("\nSaved to output/fpm_gmt_regression_coefs_FE_grid_cli.csv\n")
