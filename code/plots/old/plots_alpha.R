# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## PLOT COUNTRY ALPHA COEFFICIENTS - GLOBAL HEAT MAP ##
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# Import country-level regression coefficients produced by fpm_gmt_relation.R.
...
source(here("code", "fpm_gmt_relation.R")) # regression file and load all objects
...
hist_alpha_c2
ggsave(here("images/regression_alpha", "hist_alpha_c2.png"), plot = hist_alpha_c2, width = 8, height = 5, dpi = 300)
...
map_alpha_c1
ggsave(here("images/regression_alpha", "map_alpha_c1.png"),
...
map_alpha_c2
ggsave(here("images/regression_alpha", "map_alpha_c2.png"),
...
# For each selected country, plot the 5 (period × scenario) data points with the
# OLS fitted line: exposure_percap = alpha_c1 + alpha_c2 * T_ps
#
# Data objects from source():
#   reg_data_long -- 5 rows per country: T_ps (GMT regressor) and exposure_percap (response)
...
for (iso in countries_to_plot) {
  ...
  # Filter to this country's 5 observations and create short label
  # by stripping the "exposure_percap_fpm_" prefix from period_scenario
  df <- reg_data_long %>%
    filter(country_code_iso3 == iso) %>%
    mutate(label = str_remove(period_scenario, "exposure_percap_fpm_"))
  ...
  p <- ggplot(df, aes(x = T_ps, y = exposure_percap)) +
    # Each point is one (period × scenario) observation:
    # baseline, 2040s RCP4.5, 2040s RCP8.5, 2090s RCP4.5, 2090s RCP8.5
    geom_point(size = 3, color = "steelblue") +
    # Label each point with the shortened period_scenario value (e.g. "2000",
    # "2050_45"). vjust pushes label above the point; size controls font size.
  ...
  print(p)
  ggsave(here("images/regression_alpha", paste0("lof_", tolower(iso), ".png")),
