...
source(here("code/regression_alpha", "fpm_gmt_regression_pierce.R")) # regression file and load all objects
...
hist_alpha_c2
ggsave(here("images/regression_alpha/alpha_pierce", "hist_alpha_c2_pierce.png"), plot = hist_alpha_c2, width = 8, height = 5, dpi = 300)
...
map_alpha_c1
ggsave(here("images/regression_alpha/alpha_pierce", "map_alpha_c1_pierce.png"),
...
map_alpha_c2
ggsave(here("images/regression_alpha/alpha_pierce", "map_alpha_c2_pierce.png"),
       map_alpha_c2, width = 12, height = 6)

print("Alpha maps saved to images/regression_alpha/alpha_pierce/")
...
# For each selected country, plot all 5 observations (Pierce only) with the
...
for (iso in countries_to_plot) {
  ...
  # Filter to this country's 5 observations, join model/trajectory lookup, and
  # create point labels for all Pierce points.
  df <- reg_data_long %>%
  ...
  p <- ggplot(df, aes(x = T_ps, y = exposure_percap, color = model, shape = trajectory)) +
    ...
    geom_point(size = 3) +
    # Labels for all labelled points (Pierce).
  ...
  print(p)
  ggsave(here("images/regression_alpha/alpha_pierce", paste0("lof_", tolower(iso), "_pierce.png")),
         p, width = 7, height = 5, dpi = 300)
}


print("Line of best fit plots saved to images/regression_alpha/alpha_pierce/")
